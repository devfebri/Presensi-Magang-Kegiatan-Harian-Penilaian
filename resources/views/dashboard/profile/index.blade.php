...
@section("container")
    <div>
            ...
            <div class="dark:bg-slate-850 dark:shadow-dark-xl shadow-3xl relative mx-6 flex min-w-0 flex-auto flex-col overflow-hidden break-words rounded-2xl border-0 bg-white bg-clip-border p-4">
                <div class="-mx-3 flex flex-wrap">
                            ...
                            @if ($pemagang->foto)
                                <div class="avatar">
                                    <div class="w-20 rounded-full">
                                        <img src="{{ asset("storage/unggah/pemagang/$pemagang->foto") }}" />
                                    </div>
                                </div>
                            @else
                    ...
                    <div class="my-auto w-auto max-w-full flex-none px-3">
                        <div class="h-full">
                            <h5 class="mb-1 dark:text-white">{{ $pemagang->nama_lengkap }}</h5>
                            <p class="mb-0 text-sm font-semibold leading-normal dark:text-white dark:opacity-60">{{ $pemagang->jabatan }}</p>
                        </div>
                    </div>
        ...
        <form action="{{ route("pemagang.profile.update") }}" method="post" enctype="multipart/form-data" class="-mx-3 flex flex-wrap p-6">
            @csrf
            ...
            <div class="md:flex-0 mt-6 w-full max-w-full shrink-0 px-3 md:mt-0 md:w-4/12">
                @if ($pemagang->foto)
                    <img src="{{ asset("storage/unggah/pemagang/$pemagang->foto") }}" class="img-preview mb-3 rounded" />
                @else
