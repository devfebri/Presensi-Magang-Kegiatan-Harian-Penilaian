<aside
    ...
    <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-800">
        ...
        <a href="{{ route('pemagang.dashboard') }}" class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-emerald-500 to-sky-500 flex items-center justify-center shadow-lg flex-shrink-0">
    ...
    <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800">
        <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-full bg-gradient-to-br from-emerald-400 to-sky-500 flex items-center justify-center text-white font-bold text-sm flex-shrink-0 overflow-hidden">
                @if(Auth::guard('pemagang')->user()->foto)
                    <img src="{{ asset('storage/unggah/pemagang/' . Auth::guard('pemagang')->user()->foto) }}" class="w-full h-full object-cover" />
                @else
                    {{ strtoupper(substr(Auth::guard('pemagang')->user()->nama_lengkap, 0, 1)) }}
                @endif
            </div>
            <div class="min-w-0">
                <p class="text-sm font-semibold text-slate-800 dark:text-white truncate">{{ Auth::guard('pemagang')->user()->nama_lengkap }}</p>
        ...
        <ul class="space-y-1">
            ...
            <li>
                <a href="{{ route('pemagang.dashboard') }}"
                    ...
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 {{ Request::routeIs('pemagang.dashboard') ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-800' }}">
                        <svg class="w-4 h-4 {{ Request::routeIs('pemagang.dashboard') ? 'text-white' : 'text-sky-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <li>
                <a href="{{ route('pemagang.logbook') }}"
                    ...
                    {{ Request::routeIs('pemagang.logbook') 
                        ...
                        : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800' }}">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 {{ Request::routeIs('pemagang.logbook') ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-800' }}">
                        <svg class="w-4 h-4 {{ Request::routeIs('pemagang.logbook') ? 'text-white' : 'text-emerald-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <li>
                <a href="{{ route('pemagang.presensi') }}"
                    ...
                    {{ Request::routeIs('pemagang.presensi') 
                        ? 'bg-gradient-to-r from-emerald-500 to-emerald-600 text-white shadow-md shadow-emerald-500/25' 
                        : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800' }}">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 {{ Request::routeIs('pemagang.presensi') ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-800' }}">
                        <svg class="w-4 h-4 {{ Request::routeIs('pemagang.presensi') ? 'text-white' : 'text-emerald-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                    </div>
                    <span>Presensi</span>
            ...
            <li>
                <a href="{{ route('pemagang.izin') }}"
                    ...
                    {{ Request::routeIs(['pemagang.izin', 'pemagang.izin.create']) 
                        ...
                        : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800' }}">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 {{ Request::routeIs(['pemagang.izin', 'pemagang.izin.create']) ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-800' }}">
                        <svg class="w-4 h-4 {{ Request::routeIs(['pemagang.izin', 'pemagang.izin.create']) ? 'text-white' : 'text-amber-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <li>
                <a href="{{ route('pemagang.profile') }}"
                    ...
                    {{ Request::routeIs('pemagang.profile') 
                        ...
                        : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800' }}">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 {{ Request::routeIs('pemagang.profile') ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-800' }}">
                        <svg class="w-4 h-4 {{ Request::routeIs('pemagang.profile') ? 'text-white' : 'text-rose-500' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
