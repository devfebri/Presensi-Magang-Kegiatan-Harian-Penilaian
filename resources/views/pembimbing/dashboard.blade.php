...
<body class="min-h-screen bg-slate-100 font-sans antialiased">
    ...
    <div class="lg:ml-64 min-h-screen flex flex-col">
        ...
        <main class="flex-1 p-4 md:p-6 animate-fade-in">
            ...
            <!-- Welcome Banner -->
            <div
                class="relative bg-gradient-to-r from-purple-700 via-purple-600 to-indigo-600 rounded-2xl p-6 mb-6 overflow-hidden shadow-xl">
                <div
                    class="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full translate-x-1/3 -translate-y-1/2 pointer-events-none">
                </div>
                <div
                    class="absolute bottom-0 left-0 w-40 h-40 bg-white/5 rounded-full -translate-x-1/3 translate-y-1/2 pointer-events-none">
                </div>
                <div class="relative z-10 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
                    <div>
                        ...
                        <h2 class="font-outfit font-bold text-white text-2xl mb-1">{{ $pembimbing->nama_lengkap }}
                        </h2>
                    ...
                    @if ($namaInstansi)
                        <div
                            class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/15 border border-white/20 text-white text-sm font-medium">
                            <svg class="w-4 h-4 text-purple-200 flex-shrink-0" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                ...
                <!-- Total Pemagang -->
                <div
                    class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:shadow-md transition-shadow animate-slide-up">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span
                            class="text-xs font-semibold text-blue-500 bg-blue-50 px-2 py-1 rounded-full">Total</span>
                    </div>
                ...
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:shadow-md transition-shadow animate-slide-up"
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="text-xs font-semibold text-emerald-500 bg-emerald-50 px-2 py-1 rounded-full">Hari
                            Ini</span>
                    </div>
                ...
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:shadow-md transition-shadow animate-slide-up"
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="text-xs font-semibold text-amber-500 bg-amber-50 px-2 py-1 rounded-full">Hari
                            Ini</span>
                    </div>
            ...
            @if ($rekapBulanIni)
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                        ...
                        <p class="text-4xl font-outfit font-bold text-indigo-600">
                            {{ $rekapBulanIni->jml_kehadiran ?? 0 }}</p>
                    ...
                    <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                        ...
                        <p class="text-4xl font-outfit font-bold text-red-500">
                            {{ $rekapBulanIni->jml_terlambat ?? 0 }}</p>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <span
                        class="text-xs font-semibold text-purple-600 bg-purple-50 border border-purple-100 px-3 py-1 rounded-full">
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-100">
                                <th
                                    class="text-left px-6 py-3 text-slate-500 font-semibold text-xs uppercase tracking-wider">
                                    No</th>
                                <th
                                    class="text-left px-6 py-3 text-slate-500 font-semibold text-xs uppercase tracking-wider">
                                    Nama Pemagang</th>
                                <th
                                    class="text-left px-6 py-3 text-slate-500 font-semibold text-xs uppercase tracking-wider hidden md:table-cell">
                                    Instansi</th>
                                <th
                                    class="text-left px-6 py-3 text-slate-500 font-semibold text-xs uppercase tracking-wider">
                                    Jam Masuk</th>
                                <th
                                    class="text-left px-6 py-3 text-slate-500 font-semibold text-xs uppercase tracking-wider hidden sm:table-cell">
                                    Jam Keluar</th>
                                <th
                                    class="text-left px-6 py-3 text-slate-500 font-semibold text-xs uppercase tracking-wider">
                                    Status</th>
                        ...
                        <tbody class="divide-y divide-slate-50">
                            @forelse($daftarPemagang as $index => $item)
                                <tr class="hover:bg-slate-50/60 transition-colors">
                                    <td class="px-6 py-4 text-slate-400">{{ $daftarPemagang->firstItem() + $index }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <div>
                                    ...
                                    <td class="px-6 py-4">
                                        @if ($item->tanggal_presensi)
                                            @if ($item->jam_masuk > '08:00')
                                                <span
                                                    class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-amber-50 text-amber-600 text-xs font-semibold border border-amber-100">
                                                    <span
                                                        class="w-1.5 h-1.5 rounded-full bg-amber-500"></span>Terlambat
                                                </span>
                                            @else
                                                <span
                                                    class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-600 text-xs font-semibold border border-emerald-100">
                                                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>Hadir
                                                </span>
                                            @endif
                                        @else
                                            <span
                                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-100 text-slate-500 text-xs font-semibold">
